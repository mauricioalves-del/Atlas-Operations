...
def garantir_colunas_novas():
    ...
    # Mesmo motivo do bloco acima (mesma lição do bug de 19/08/2026): colunas
    # novas de Usuario (integrações pessoais Gmail/Slack - ver
    # app/integracoes_pessoais.py) também precisam ficar aqui no topo, porque
    # deps.obter_usuario_atual() faz `db.query(models.Usuario).filter_by(...)`
    # em TODA requisição autenticada - inclusive a primeira, ainda dentro do
    # boot - e um SELECT do ORM sempre pede todas as colunas que o modelo
    # Python declara, mesmo as recém-adicionadas ao models.py.
    if inspecao.has_table("usuarios"):
        colunas_usuarios = {c["name"] for c in inspecao.get_columns("usuarios")}
        if "google_refresh_token" not in colunas_usuarios:
            with engine.connect() as conn:
                conn.execute(text("ALTER TABLE usuarios ADD COLUMN google_conectado_email VARCHAR"))
                conn.execute(text("ALTER TABLE usuarios ADD COLUMN google_refresh_token TEXT"))
                conn.execute(text("ALTER TABLE usuarios ADD COLUMN slack_conectado_user_id VARCHAR"))
                conn.execute(text("ALTER TABLE usuarios ADD COLUMN slack_conectado_team_id VARCHAR"))
                conn.execute(text("ALTER TABLE usuarios ADD COLUMN slack_user_token TEXT"))
                conn.commit()
