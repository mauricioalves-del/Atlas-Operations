...
def historico_sku_fechamento(sku: str, almoxarifado: str | None = None, usuario: models.Usuario = Depends(obter_usuario_atual), db: Session = Depends(get_db)):
    ...
    for i in itens:
        ...
        pontos.append({
            ...
            # id do item de fechamento e da Divergencia linkada (quando divergente) - usado pelo
            # front (painel "Acompanhamento do item" do modal de justificativa, 14/08/2026) pra
            # deixar cada linha da linha do tempo clicável e abrir o diagnóstico completo daquela
            # divergência (mesma tela de detalhe usada em todo o resto do app).
            "item_fechamento_id": i.id, "divergencia_id": i.divergencia_id,
        })
