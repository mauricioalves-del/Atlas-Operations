...
def listar(
    ...
    if busca:
        # Busca por SKU (parcial) ou pela descrição cadastrada do produto -
        # como a lista é paginada no servidor (diferente dos modais "Ver
        # todos", que trazem tudo de uma vez), o filtro precisa entrar na
        # própria query em vez de ser feito no JS sobre a página carregada.
        termo = f"%{busca.strip()}%"
        skus_por_descricao = {
            p.sku for p in db.query(models.Produto.sku).filter(models.Produto.descricao.ilike(termo)).all()
        }
        condicoes_busca = [models.Divergencia.sku.ilike(termo)]
